...
function inicializarAsientos(filas = FILAS, columnas = COLUMNAS) {
    const sala = [];
...
function imprimirAsientos(sala) {
    console.log("\n=== ESTADO DE LA SALA ===");
    let cabecera = "    ";
...
function buscarDosAdyacentes(sala) {
    for (let f = 0; f < sala.length; f++) {
...
function mostrarEscenario(titulo, sala) {
    console.log(`\n########## ${titulo} ##########`);
    ...
    if (adyacentes) {
        ...
        console.log(`Primeros dos asientos adyacentes libres: fila ${a[0] + 1}, columnas ${a[1] + 1} y ${b[1] + 1}`);
    }
    else {
